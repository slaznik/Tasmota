...
struct mesh_first_header_bytes
{ // TODO: evaluate random 4-byte-value of pre-packet
  uint8_t raw[15];
} __attribute__((packed));
;
...
enum MESH_Commands
{                   // commands useable in console or rules
  ...
  CMND_MESH_CHANNEL,
  CMND_MESH_SEND, // send command to a peer
}; // set wifi channel on node (the broker gets it automatically from the AP)
...
void MESHsendTime(void)
{ // Only from broker to nodes
...
void MESHsendPeerList(void)
{ // We send this list only to the peers, that can directly receive it
  MESH.sendPacket.counter++;
...
void MESHHexStringToBytes(char *_string, uint8_t _MAC[])
{ // uppercase
