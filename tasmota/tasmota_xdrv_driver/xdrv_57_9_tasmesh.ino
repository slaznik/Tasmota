...
void CB_MESHDataReceived(uint8_t *MAC, uint8_t *packet, uint8_t len)
{
  ...
  if (memcmp(_recvPacket->receiver, MESH.sendPacket.sender, 6) != 0)
  { // MESH.sendPacket.sender simply stores the MAC of the node
...
bool MESHinterceptMQTTonBroker(char *_topic, uint8_t *_data, unsigned int data_len)
{
  ...
  char stopic[TOPSZ];
  AddLog(LOG_LEVEL_DEBUG, PSTR("MSH: Intercept topic %s with data %s"), _topic, _data);
  for (auto &_peer : MESH.peers)
  {
    ...
    if (memcmp(_topic, stopic, strlen(stopic)) == 0)
    {
      ...
      AddLog(LOG_LEVEL_DEBUG, PSTR("MSH: Intercept topic with data '%s'"), MESH.sendPacket.payload);
...
void MESHstartNode(int32_t _channel, uint8_t _role)
{              // we need a running broker with a known channel at that moment
...
void MESHstartBroker(void)
{ // Must be called after WiFi is initialized!! Rule - on system#boot do meshbroker endon
#ifdef ESP32
...
void MESHEverySecond(void)
{
  ...
  if (_second % 5 == 0)
  {
    if ((MESH.flags.nodeWantsTime == 1) || (_second % 30 == 0))
    { // every 5 seconds on demand or every 30 seconds anyway
      MESHsendTime();
...
void MESHevery50MSecond(void)
{
  ...
  if (MESH.packetToResend.size() > 0)
  {
    ...
    if (MESH.packetToResend.front().TTL > 0)
    {
      ...
      if (memcmp(MESH.packetToResend.front().sender, MESH.broker, 6) != 0)
      { // do not send back the packet to the broker
  ...
  {
    MESHencryptPayload(&MESH.packetToConsume.front(), 0);
    AddLog(LOG_LEVEL_DEBUG, PSTR("MSH: received message of type: %d"), MESH.packetToConsume.front().type);
    switch (MESH.packetToConsume.front().type)
    {
...
const char kMeshCommands[] PROGMEM = "Mesh|" // Prefix
                                     "Broker|Node|Peer|Channel|Interval|Send";

void (*const MeshCommand[])(void) PROGMEM = {
    &CmndMeshBroker, &CmndMeshNode, &CmndMeshPeer, &CmndMeshChannel, &CmndMeshInterval, &CmndMeshSend};
...
void CmndMeshNode(void)
{
  ...
  if (XdrvMailbox.data_len > 0)
  {
    ...
    {
      XdrvMailbox.index = 1;
    } // Everything not 0 is a full node
...
void CmndMeshSend(void)
{
#ifdef ESP32
  if (XdrvMailbox.data_len > 0)
  {
    // find delimiter that delimits topic and data
    char *delimiterPtr = strchr(XdrvMailbox.data, '+');
    AddLog(LOG_LEVEL_DEBUG, PSTR("MSH: Intercept payload topic+data %s"), XdrvMailbox.data);
    ...
    {
      GetTopic_P(stopic, CMND, _peer.topic, PSTR("")); // cmnd/topic/
      if (memcmp(XdrvMailbox.data, stopic, strlen(stopic)) == 0)
      {
        char _destMAC[18];
        ToHex_P(_peer.MAC, 6, _destMAC, 18, ':');
        AddLog(LOG_LEVEL_DEBUG, PSTR("MSH: Found matching peer %s"), _destMAC);

        if (XdrvMailbox.data_len > MESH_PAYLOAD_SIZE)
        {
          AddLog(LOG_LEVEL_DEBUG, PSTR("MSH: Intercept payload oversized %d"), XdrvMailbox.data_len);
          return;
        }

        memcpy(MESH.sendPacket.receiver, _peer.MAC, 6);
        memcpy(MESH.sendPacket.payload, XdrvMailbox.data, XdrvMailbox.data_len);
        MESH.sendPacket.chunkSize = XdrvMailbox.data_len;
        MESH.sendPacket.chunks = 1;
        MESH.sendPacket.type = PACKET_TYPE_COMMAND;
        MESH.sendPacket.senderTime = Rtc.utc_time;
        MESHsendPacket(&MESH.sendPacket);
        AddLog(LOG_LEVEL_DEBUG, PSTR("MSH: Sent payload to peer '%s'"), _destMAC);
      }
    }
  }
